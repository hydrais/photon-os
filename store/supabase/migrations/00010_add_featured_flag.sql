ALTER TABLE store_apps ADD COLUMN featured BOOLEAN DEFAULT FALSE;

-- Create partial index for faster featured app queries
CREATE INDEX idx_store_apps_featured ON store_apps(featured) WHERE featured = TRUE;

-- Drop existing function (return type is changing)
DROP FUNCTION IF EXISTS search_store_apps(TEXT);

-- Recreate search function with featured field
...
RETURNS TABLE (
  ...
  average_rating DECIMAL, review_count INTEGER, featured BOOLEAN
) AS $$
BEGIN
  RETURN QUERY
  SELECT
    ...
    sa.average_rating, sa.review_count, sa.featured
  FROM store_apps sa
